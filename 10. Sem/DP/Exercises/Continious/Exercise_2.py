...
def EGM_vectorized (sol,t,par):
    ...
    m_next = par.R * par.grid_a[:,None] + par.eps[None,:] # 1. Find future m
    c_next = interp(m_next) # 2. Find c_next using interpolation of next period solution
    
    EU_next = np.sum(par.eps_w[None,:] * marg_util(c_next,par),axis=1) # 3. Find expected marginal utility of next period consumption
    
    c_now = inv_marg_util(par.R * par.beta * EU_next,par) # 4. Find optimal consumption using inverted Euler
