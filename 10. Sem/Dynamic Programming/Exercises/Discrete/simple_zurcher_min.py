...
def bellman(V_next, par, taste_shocks = 'None', stochastic_transition = False):
    ...
    for  x in par.grid: # Loop over states
        ...
        if stochastic_transition == False: # Exercise 1
            EV_keep = EV_deterministic(0, x, V_next, par) # Fill in
            EV_replace = EV_deterministic(1, x, V_next, par) # Fill in
        else: #Exercise 2
            EV_keep = EV_stochastic(0, x, V_next, par) # Fill in
            EV_replace = EV_stochastic(1, x, V_next, par) # Fill in
        ...
        # Exercise 4
            ...
            V_now[x] = logsum
            pk[x] = np.exp((value_replace-maxV)/par.sigma_eps) / (np.exp((value_keep-maxV)/par.sigma_eps)  +  np.exp((value_replace-maxV)/par.sigma_eps))
...
def EV_stochastic(d, x, V_next, par):
    ...
    (par.p).
    """
    EV = 0 # Initialize expected value
    for p, m in zip(par.p, par.transition):
        x_next = x + m  # Calculate next state
        x_next = np.fmin(x_next, par.n-1) # Ensure that x_next is within grid
        EV += p * V_next[x_next] # Calculate expected value
    return EV
...
def bellman_vector(V_next, par):
    ...
    value_keep = util(0, par.grid, par) + par.beta * np.dot(par.P1, V_next) # Calculate value of keep
    value_replace = util(1, par.grid, par) + par.beta * np.dot(par.P2, V_next) # Calculate value of replace
