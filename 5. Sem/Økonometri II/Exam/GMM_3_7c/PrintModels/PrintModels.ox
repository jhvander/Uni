...
PrintModels::GetResultsARFIMA()
  ...
  for (i = 0; i < sizeof(m_oModels); i++)
    ...
    decl cpall = rows(deleteifr(mEstTemp, mEstTemp .== 0))+1; //oModel.m_cARMA ? int(sumc(oModel.m_vIsFreePar[1 : oModel.m_cARMA])) : 0;

    // Get the information criteria
    mIC[0][i] = (-2 * mLogLik[0][i] + 2 * cpall                   ) /mT[0][i];  // AIC
    mIC[1][i] = (-2 * mLogLik[0][i] + 2 * cpall*log(log(mT[0][i]))) /mT[0][i];  // HQ
    mIC[2][i] = (-2 * mLogLik[0][i] +     cpall*log(mT[0][i])     ) /mT[0][i];  // SQ/BIC
