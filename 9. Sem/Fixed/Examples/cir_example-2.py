...
for i in range(0,N_simul):
    ...
    for idx in range(idx_cap_begin,idx_cap_end):
        idx_announcement = idx*int((alpha*M_simul_cap)/T_cap_end)
        print(idx_announcement)
        # idx_payment = (idx+1)*int((alpha*M_simul_cap)/T_cap_end)
        p_cap = fid.zcb_price_cir(r_simul_cap[idx_announcement],a,b,sigma,alpha)
        delta = (idx+1)*alpha/idx_announcement
        chi[idx-1,i] = np.exp(-delta*sum(r_simul_cap[0:idx_announcement]))*(1+alpha*strike)*max(1/(1+alpha*strike)-p_cap,0)
...
# fig.savefig("C:/Jacob/Uni_of_CPH/FID/FID_E2024/Examples/cir_yield_curve.pdf
plt.show()
...
# fig.savefig("C:/Jacob/Uni_of_CPH/FID/FID_E2024/Examples/cir_simul.pdf
plt.show()
